# OmniWM — niri-style tiling window manager (BarutSRB/OmniWM).
#
# Managed by nix-darwin:
#   - Config generated by flake → installed to ~/.config/omniwm/settings.toml
#     at activation (via system.activationScripts.extraActivation).
#   - LaunchAgent keeps OmniWM alive at login.
#
# CRITICAL: OmniWM REQUIRES the FULL config (all keys + UUID ids on
# appRules/workspaces). A partial config is treated as corrupt and
# OmniWM falls back to defaults + rewrites the file (verified 2026-08-30).
# This config was captured from the working ~/.config/omniwm/settings.toml
# (with vim-style HJKL focus/move hotkeys added).
#
# To apply config changes: rebuild + darwin-rebuild switch, or just
#   launchctl kickstart -k gui/$UID/org.nixos.omniwm
{
  config,
  lib,
  pkgs,
...
in {
  ...
  # Install config at activation (root → chown to user).
  # NOTE: custom activationScripts names are NOT reliably executed by
  # nix-darwin; extraActivation (predefined) is guaranteed to run.
  system.activationScripts.extraActivation.text = lib.mkAfter ''
    mkdir -p ${shared.home}/.config/omniwm/
    rm -f ${shared.home}/.config/omniwm/settings.toml
    install -m 0644 -o ${shared.username} -g staff ${omniCfg} ${shared.home}/.config/omniwm/settings.toml
  '';

  # LaunchAgent: start OmniWM at login, keep alive
  launchd.user.agents.omniwm = {
    serviceConfig = {
      ProgramArguments = ["${pkgs.omniwm}/Applications/OmniWM.app/Contents/MacOS/OmniWM"];
      RunAtLoad = true;
      KeepAlive = true;
      ProcessType = "Interactive";
      StandardOutPath = "/tmp/omniwm.log";
      StandardErrorPath = "/tmp/omniwm.log";
    };
  };
}
